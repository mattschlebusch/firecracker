...
pub enum CpuConfigurationTypes {
...
impl Default for CpuConfigurationTypes {
    /// Returns a "none" type for CPU configuration.
    /// Firecracker and KVM defaults will be used to configure vCPUs.
    fn default() -> Self {
        CpuConfigurationTypes::Static(CpuFeaturesTemplate::None)
    }
}

impl CpuConfigurationTypes {
    ...
    pub fn from(
            ...
            && custom_config.is_some()
        {
            ...
            return Ok(CpuConfigurationTypes::Static(static_template));
        } else if let Some(custom_config) = custom_config {
            return Ok(CpuConfigurationTypes::Custom(custom_config));
        }

        Ok(CpuConfigurationTypes::Static(CpuFeaturesTemplate::None))
    }
}
