...
use crate::guest_config::templates::{
    create_guest_cpu_config, CpuConfiguration, CpuConfigurationTypes, CpuTemplate, GuestConfigError,
};
...
fn build_guest_cpu_config(
    ...
    vm: &Vm,
    vcpu: &Vcpu,
) -> Result<CpuConfigurationTypes, GuestConfigError> {
    ...
    CpuConfigurationTypes::from(Some(vm_resources.vm_config.config.cpu_template), cpu_config)
}
