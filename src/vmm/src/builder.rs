    ...
    fn fmt(&self, f: &mut Formatter) -> std::fmt::Result {
        ...
        match self {
            ApplyCpuTemplate(err) => {
                write!(
                    f,
                    "Unable to successfully apply custom CPU to create configuration usable for \
                     guest vCPUs: {:?}",
                    err
                )
            }
                ...
                create_guest_cpu_config(
                    ...
                    &X86_64CpuConfiguration {
                        cpuid: Cpuid::try_from(RawCpuid::from(vm.supported_cpuid().clone()))
                            .unwrap(),
                        msrs: get_msr_values(vcpus.get(0), template),
                    },
                )
            ...
            vm.guest_cpu_config = Some(
                create_guest_cpu_config(
                    ...
                    &Aarch64CpuConfiguration {
                        regs: get_reg_values(vcpus.get(0), template),
                    },
                )
                .map_err(StartMicrovmError::ApplyCpuTemplate)?,
            );
...
#[cfg(target_arch = "x86_64")]
#[allow(unused)]
fn get_msr_values(_vcpu: Option<&Vcpu>, _template: &X86_64CpuTemplate) -> HashMap<u32, u64> {
...
#[cfg(target_arch = "aarch64")]
#[allow(unused)]
fn get_reg_values(_vcpu: Option<&Vcpu>, _template: &Aarch64CpuTemplate) -> HashMap<u64, u128> {
