...
pub fn bitfield(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let bits_len = if let Ok(data) = DataType::from_str(struct_data_type.to_string().as_str()) {
        data
    } else {
    ...
    while let Some(field_token_slice) = fields_slice_iter.next() {
        ...
        match field_token_slice {
            ...
            {
                check_ident!(ident);
                let (start, stop) = index_field!(start_token, stop_token);
            ...
            {
                check_ident!(ident);
                let index = index_flag!(index_token);
